...
// The SerialFunc type is an adapter to allow the use of ordinary
// function as Serial mutator.
type SerialFunc func(context.Context, *ent.SerialMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SerialFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.SerialMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SerialMutation", m)
	}
	return f(ctx, mv)
}
