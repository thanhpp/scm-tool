...
// QueryItemSerial chains the current query on the "item_serial" edge.
func (iq *ItemQuery) QueryItemSerial() *SerialQuery {
	query := &SerialQuery{config: iq.config}
...
func (iq *ItemQuery) Clone() *ItemQuery {
	...
	return &ItemQuery{
		...
		withItemSerial: iq.withItemSerial.Clone(),
...
// WithItemSerial tells the query-builder to eager-load the nodes that are connected to
// the "item_serial" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *ItemQuery) WithItemSerial(opts ...func(*SerialQuery)) *ItemQuery {
	query := &SerialQuery{config: iq.config}
	for _, opt := range opts {
		opt(query)
	}
	iq.withItemSerial = query
	return iq
}
...
func (iq *ItemQuery) sqlAll(ctx context.Context) ([]*Item, error) {
	...
	if query := iq.withItemSerial; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uuid.UUID]*Item)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.ItemSerial = []*Serial{}
		}
		query.Where(predicate.Serial(func(s *sql.Selector) {
			s.Where(sql.InValues(item.ItemSerialColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "item_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.ItemSerial = append(node.Edges.ItemSerial, n)
		}
	}
